...
def list_available_csvs() -> str:
    """Resource that lists all CSV files currently available for processing."""
    import os
    from tools.basic import CSV_FOLDER
    files = sorted([f for f in os.listdir(CSV_FOLDER) if f.endswith(".csv")])
    if not files:
        return "No CSV files found in project root /csv folder."
    return "Available CSV files:\n" + "\n".join([f"- {f}" for f in files])

@mcp.resource("csv://{filename}")
def read_csv_resource(filename: str) -> str:
    """Resource to read the raw content of a specific CSV file."""
    from tools.basic import CSV_FOLDER
...
@mcp.prompt()
def analyze_and_provision(filename: str = None) -> str:
    """
    A guided workflow to analyze a CSV and provision entitlements to an Okta app.
    
    If filename is not provided, please list the files first.
    """
    if not filename:
        return "I want to start the Okta entitlement provisioning workflow. Please show me a list of available CSV files and ask me which one to process."
    
    return (
        f"I want to process the file '{filename}' for Okta entitlements. "
        "Please follow these steps:\n"
        "1. Run `validate_csv_preflight` on it to ensure data quality.\n"
        "2. If valid, run `analyze_csv_for_entitlements` to extract the structure.\n"
        "3. Once analyzed, ask me for the Okta Application ID to proceed with Stage 2."
    )
...
@mcp.tool()
async def validate_csv_preflight(file: str) -> str:
    """Validate a CSV file for common errors before Okta processing."""
    return await basic.validate_csv_preflight({"file": file})
